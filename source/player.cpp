...
public:
    ...
    TestUnit(int x, int y, int spriteId) : Unit(x, y, spriteId) { }
    ...
    for (size_t x = 0; x < 8; ++x) {
        for (size_t y = 0; y < 6; ++y) {
            ...
            if (unit != nullptr) {
                ...
                while (at(x - relativePosition.x, y) == unit) { ++relativePosition.x; }
                relativePosition.x--;

                while (at(x - relativePosition.x, y - relativePosition.y) == unit) { ++relativePosition.y; }
                relativePosition.y--;
